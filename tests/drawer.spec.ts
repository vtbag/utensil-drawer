...
test('two chained calls', async ({ page, browserName }) => {
	...
	await new Promise<void>(resolve => setTimeout(resolve, 1000));
	expect(text).toBe("  true  hi  true  hi ho update1  hi ho update2  hi ho update1b update2b done done ::view-transition-group(root) ::view-transition-group(root) finished finished");
});
...
test('one plus two chained calls', async ({ page, browserName }) => {
	...
	await new Promise<void>(resolve => setTimeout(resolve, 1500));
	expect(text).toBe("  true  hi update1  hi  true  ho update1b done ::view-transition-group(root)  true  ha finished update2  ho he ha hu update3  ho he ha hu update2b update3b done done ::view-transition-group(root) ready2  ho he ha hu x2 ::view-transition-group(root) finished finished");
});
