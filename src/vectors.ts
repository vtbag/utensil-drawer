...
*/
export function setVectors(instructions: Inst[] = [{ pattern: ".*", props: ["x", "y", "width", "height"] }],
	where: "pseudo" | "root" | "both" = "both"): void {
	...
	document.getAnimations().forEach((animation: any) => {
		...
		const from: Params = { width: frames[0]?.width || 0, height: frames[0]?.height || 0, matrix: new DOMMatrixReadOnly(frames[0]?.transform || 'none') };

		const end = frames[frames.length - 1];
		let to: Params;
		if (end.width === "auto") { // Chrome bug
			...
			to = { width: last.width, height: last.height, matrix: new DOMMatrixReadOnly(last.transform || 'none') };
			animation.currentTime = now;
		} else {
			to = { width: end.width || 0, height: end.height || 0, matrix: new DOMMatrixReadOnly(end.transform || 'none') };
		}

		if (where === "root" || where === "both") {
			styles.push(":root {");
			emit.includes("x") && styles.push(`	--vtbag-vector-${group}-from-x: ${from.matrix.e}px;
	--vtbag-vector-${group}-to-x: ${to.matrix.e}px;`);
			emit.includes("y") && styles.push(`	--vtbag-vector-${group}-from-y: ${from.matrix.f}px;
	--vtbag-vector-${group}-to-y: ${to.matrix.f}px;`);
			emit.includes("width") && styles.push(`	--vtbag-vector-${group}-from-width: ${from.width}px;
	--vtbag-vector-${group}-to-width: ${to.width}px;`);
			emit.includes("height") && styles.push(`	--vtbag-vector-${group}-from-width: ${from.height}px;
	--vtbag-vector-${group}-to-width: ${to.height}px;`);
			styles.push("}");
		}
		if (where === "pseudo" || where === "both") {
			styles.push(`::view-transition-group(${group}) {`);
			emit.includes("x") && styles.push(`	--vtbag-vector-from-x: ${from.matrix.e}px;
	--vtbag-vector-to-x: ${to.matrix.e}px;`);
			emit.includes("y") && styles.push(`	--vtbag-vector-from-y: ${from.matrix.f}px;
	--vtbag-vector-to-y: ${to.matrix.f}px;`);
			emit.includes("width") && styles.push(`	--vtbag-vector-from-width: ${from.width}px;
	--vtbag-vector-to-width: ${to.width}px;`);
			emit.includes("height") && styles.push(`	--vtbag-vector-from-height: ${from.height}px;
